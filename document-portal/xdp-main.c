                       ...
                       GError **error)
{
  ...
  char *app_rel_path, *usr_rel_path;
  g_autofree char *app_path = NULL;

  if (!validate_fd_common (fd, st_buf, sandboxed_path_buffer, error))
    return FALSE;

  app_rel_path = strstr (sandboxed_path_buffer, "/app");
  usr_rel_path = strstr (sandboxed_path_buffer, "/usr");

  if (app_rel_path != NULL)
    {
      app_rel_path += strlen ("/app");
      app_path = resolve_flatpak_app_path (app_rel_path, app_id);
    }
  else if (usr_rel_path != NULL)
    {
      usr_rel_path += strlen ("/usr");
      app_path = resolve_flatpak_usr_path (usr_rel_path, app_id);
    }
